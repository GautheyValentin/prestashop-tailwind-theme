{if $homeslider.slides}
  <div x-data='{literal}{items: {/literal}{$homeslider.slides|count}{literal}, active: 0}{/literal}' id="carousel"
    ...
    <div class="block top-0 left-0 h-full w-full" aria-label="{l s='Carousel buttons' d='Shop.Theme.Global'}">
      <span class="absolute top-1/2 left-0 cursor-pointer" aria-hidden="true"
      ...
      <span class="absolute top-1/2 right-0 cursor-pointer" aria-hidden="true"
    ...
    <ol class="absolute bottom-5 flex" style="left: calc(50% - ({$homeslider.slides|count} * 14px) / 2);">
      {foreach from=$homeslider.slides item=slide name='homeslider' key=key}
        {capture assign=active}active === {$key}{/capture}

        <li class="slider-rounder rounded-full w-3 h-3 border border-solid border-red-550 cursor-pointer transition-all"
